...
- **Rich Note Creation**: Creates formatted notes in CMS with summary and key points
- **Task Generation**: Automatically creates tasks for action items and commitments, linked to the meeting note

## Requirements

- `cms-cli` - Install globally: `npm install -g cms-cli`
- `apiKey` - Go to `https://cms.com/settings/api-webhooks` to generate one
...
2. Edit `.env` and replace the placeholders:
   - `<SET_YOUR_CMS_API>` with your CMS API key
...
4. Sync the app to your CMS workspace:
...
After syncing, configure the environment variables in your CMS workspace:
1. Go to Settings → Apps → AI Meeting Transcript
2. Set the following environment variables:
   - `CMS_API_KEY` - Your CMS API key
   - `CMS_API_URL` - Your CMS instance URL (e.g., `https://api.cms.com` or `http://localhost:3000` for local development)
   - `OPENAI_API_KEY` - Your OpenAI API key

**Important**: `CMS_API_URL` is required and must be set to your CMS instance URL. For local development, use `http://localhost:3000`. For production, use your actual CMS instance URL.
...
- Hosted: `POST https://your-cms-instance.com/s/webhook/transcript`
...
```bash
curl -X POST https://your-cms-instance.com/s/webhook/transcript \
...
2. **Note Creation**: A formatted note is created in CMS with:
   - Meeting summary
   - Key discussion points
...
2. Configure it to POST to: `https://your-cms-instance.com/s/webhook/transcript`
