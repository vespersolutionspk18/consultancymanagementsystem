...
export const main = async (): Promise<object> => {
  ...
  log(
    'Available environment variables: ' +
      Object.keys(process.env)
        .filter((k) => k.includes('CMS') || k.includes('WEBMETIC'))
        .join(', '),
  );
  ...
  const CMS_API_BASE_URL =
    ...
    process.env.CMS_API_URL !== undefined
      ? process.env.CMS_API_URL
      : 'http://localhost:3000';

  // REST URL for data operations
  const CMS_API_URL = `${CMS_API_BASE_URL}/rest`;

  log('WEBMETIC_API_KEY: ' + (WEBMETIC_API_KEY ? '***SET***' : 'MISSING'));
  log('WEBMETIC_DOMAIN: ' + (WEBMETIC_DOMAIN || 'MISSING'));
  log('CMS_API_KEY: ' + (CMS_API_KEY ? '***SET***' : 'MISSING'));
  log('CMS_API_BASE_URL: ' + CMS_API_BASE_URL);
  log('CMS_API_URL (REST): ' + CMS_API_URL);

  if (!WEBMETIC_API_KEY || !WEBMETIC_DOMAIN || !CMS_API_KEY) {
    return {
      success: false,
  ...
  try {
    ...
    await ensureWebsiteLeadObjectExists(CMS_API_KEY, CMS_API_BASE_URL);

    // Then, ensure all fields exist (create via GraphQL if missing)
    await ensureWebsiteLeadFieldsExist(CMS_API_KEY, CMS_API_BASE_URL);
  } catch (error: any) {
  ...
  try {
    ...
    for (const company of companies) {
      try {
        ...
        // 2a. Find or create Company in CMS
        ...
        const existingCompaniesOptions = {
          ...
          url: `${CMS_API_URL}/companies?filter=domainName.primaryLinkUrl%5Beq%5D%3A%22${encodeURIComponent(domain)}%22`,
        };
        ...
        // If you need these fields, you'll need to create custom fields in CMS first
        ...
        for (const session of company.sessions) {
          try {
            ...
            const existingLeadsOptions = {
              ...
              url: `${CMS_API_URL}/websiteLeads?filter=name%5Beq%5D%3A%22${encodeURIComponent(leadName)}%22`,
            };
