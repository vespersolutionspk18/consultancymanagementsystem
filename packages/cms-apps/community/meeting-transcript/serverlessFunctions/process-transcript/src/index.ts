...
const lookupWorkspaceMemberByName = async (
  ...
  const { apiKey, baseUrl } = getCMSApiConfig();
...
const createTasksFromActionItems = async (
  ...
  for (const actionItem of actionItems) {
    try {
      ...
      const task = await createTaskInCMS({
        ...actionItem,
...
export const main = async (
  ...
  try {
    ...
    log('📄 Creating note in CMS CRM...');
    const note = await createNoteInCMS(
      analysis.summary,
      analysis.keyPoints,
      relatedPersonId,
      meetingTitle,
