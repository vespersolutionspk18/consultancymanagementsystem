...
const CMS_API_URL: string =
  process.env.CMS_API_URL ?? 'https://api.cms.com';
...
const returnWorkspaceMemberObjectId = async (): Promise<string> => {
  const options = {
    ...
    url: `${CMS_API_URL}/rest/metadata/objects`,
  };
...
const updateUpdatedByFieldValue = async (
  ...
  const options = {
    ...
    url: `${CMS_API_URL}/rest/${objectName}/${recordId}`,
    data: {
...
export const main = async (params: {
  ...
  if (CMS_API_KEY === '') {
    console.error('You must specify a valid CMS_API_KEY');
    return {};
  }
  try {
