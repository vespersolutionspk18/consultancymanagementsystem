# CMS Browser Extension

A Chrome browser extension for capturing LinkedIn profiles (people and companies) directly into CMS CRM. This is a basic **v0** focused mostly on establishing a solid architectural foundation.

## Overview

This extension integrates with LinkedIn to extract profile information and create records in CMS CRM. It uses **WXT** as the framework - initially tried Plasmo, but found WXT to be significantly better due to its extensibility and closer alignment with the Chrome extension APIs, providing more control and flexibility.
...
1. **`cms-browser-extension`** - The main extension package (WXT + React)
2. **`cms-apps/browser-extension`** - Serverless functions for API interactions
...
- **Popup** (`src/entrypoints/popup/`)
  ...
  - Provides buttons to save person/company to CMS
...
Located in `packages/cms-apps/browser-extension/serverlessFunctions/`:

- **`/s/create/person`** - Creates a new person record in CMS
- **`/s/create/company`** - Creates a new company record in CMS
...
- CMS CLI installed globally: `npm install -g cms-cli`
- API key from CMS: https://cms.com/settings/api-webhooks

### Setup
   ```
1. **Configure environment variables:**
   - Set `CMS_API_URL` in the serverless function configuration
   - Set `CMS_API_KEY` (marked as secret) in the serverless function configuration
...
npx nx run dev cms-browser-extension

# Build for production
npx nx run build cms-browser-extension

# Package extension for distribution
npx nx run package cms-browser-extension
```
...
2. **Load extension in Chrome:**
   ...
   - Click "Load unpacked"
   - Select `packages/cms-browser-extension/dist/chrome-mv3-dev/`
...
3. **Test on LinkedIn:**
   ...
   - The "Add to CMS" button should appear in the profile header
   - Click the button to open the popup and save to CMS

### Project Structure

```
packages/cms-browser-extension/
├── src/
│   ├── common/
...
✅ Create person records in CMS
✅ Create company records in CMS

## Planned Features

- [ ] Provide a way to have API key and custom remote URLs.
- [ ] Detect if record already exists and prevent duplicates
- [ ] Open existing CMS record when clicked (instead of creating duplicate)
