...
import { type stripeCustomer, type stripeEvent, type stripeStatus, type cmsObject } from './types';
...
): Promise<cmsObject | undefined> => {
  const options = {
    method: 'GET',
    headers: {
      Authorization: `Bearer ${CMS_API_KEY}`,
    },
    url: `${CMS_API_URL}/metadata/objects`,
  };
  try {
    const response = await axios.request(options);
    if (response.status === 200) {
      const companyObject = response.data.data.objects.find(
        (object: cmsObject) => object.nameSingular === objectSingularName,
      );
      return (companyObject as cmsObject) ?? ({} as cmsObject);
    }
  } catch (error) {
...
const createFields = async (objectId: string, fieldName: string) => {
  ...
  const options = {
    ...
    url: `${CMS_API_URL}/metadata/fields`,
    data: data,
  };
  try {
...
const checkIfCompanyExistsInCMS = async (
  ...
  const options = {
    ...
    url: `${CMS_API_URL}/companies?filter=name%5Beq%5D%3A%22${name}%22`,
  };
  try {
    const response = await axios(options);
    return response.status === 200 &&
...
const updateCMSCompany = async (
  ...
  const options = {
    ...
    url: `${CMS_API_URL}/companies/${companyId}`,
    data: {
      seats: seats,
      subStatus: subStatus,
    },
...
const checkIfStripePersonExistsInCMS = async (email: string | null) => {
  const options = {
    method: 'GET',
    headers: {
      Authorization: `Bearer ${CMS_API_KEY}`,
    ...
    url: `${CMS_API_URL}/people?filter=emails.primaryEmail%5Beq%5D%3A%22${email}%22`, // mail is unique by default so there can be only 1 person with given mail
  };
  try {
    const response = await axios.request(options);
    return response.status === 200 &&
...
export const main = async (
  ...
  try {
    ...
    const companyObject = await getCMSObjectData('company');
    if (
    ...
    const personObject = await getCMSObjectData('person');
    if (
    ...
    const cmsCompanyId: string | undefined =
      await checkIfCompanyExistsInCMS(stripeCustomer?.businessName);
    const seats: number =
    ...
    let updatedCMSCompanyId: string | undefined;
    if (cmsCompanyId === '') {
      const cmsCompanyCreated: string | undefined =
        await createCMSCompany(
          stripeCustomer?.businessName,
          seats,
          stripe.data.object.status.toUpperCase(),
        );
      if (cmsCompanyCreated === '') {
        throw new Error('Creation of Stripe customer in CMS failed');
      } else {
        console.log('Creation of Stripe customer in CMS succeeded');
        updatedCMSCompanyId = cmsCompanyCreated;
      }
    } else {
      const cmsCompanyUpdated: boolean | undefined =
        await updateCMSCompany(
          cmsCompanyId,
          seats,
          stripe.data.object.status.toUpperCase() as stripeStatus,
        );
      if (!cmsCompanyUpdated) {
        throw new Error('Update of Stripe customer in CMS failed');
      } else {
        console.log('Update of Stripe customer in CMS succeeded');
        updatedCMSCompanyId = cmsCompanyId;
      }
    }

    if (updatedCMSCompanyId === undefined || updatedCMSCompanyId === '') {
      throw new Error('CMSCompanyId not found');
    } else {
      const stripeCustomerInCMS: string | undefined =
        await checkIfStripePersonExistsInCMS(stripeCustomer.email);
      if (stripeCustomerInCMS === '') {
        ...
        const addedStripePersonToCMS: boolean | undefined =
          await addCMSPerson(
            firstName,
            lastName,
        ...
        if (!addedStripePersonToCMS) {
          throw new Error('Adding Stripe person to CMS failed');
        } else {
          console.log('Stripe person was added to CMS');
        }
      } else if (stripeCustomerInCMS !== undefined) {
        const updatedStripePersonInCMS: boolean | undefined =
          await updateCMSPerson(
            stripeCustomerInCMS,
            seats,
            stripe.data.object.status.toUpperCase() as stripeStatus,
          );
        if (!updatedStripePersonInCMS) {
          throw new Error('Update of Stripe person in CMS failed');
        } else {
          console.log('Update of Stripe person in CMS succeeded');
        }
      } else {
