...
General-purpose rollup engine for CMS workspaces. The bundle ships a
...
- an API key with access to your workspace. Generate one at
  `https://cms.com/settings/api-webhooks`.
...
- To script the setup, export `CMS_API_KEY` (and `CMS_METADATA_BASE_URL` if you are not targeting `http://localhost:3000/rest/metadata`) and run `yarn setup:metadata`. The helper script calls the Metadata API to ensure each field exists on the Company object, skipping anything already provisioned.
...
2. **Configure environment variables**
   ...
   - Provide values for the manifest-defined keys:
     - `CMS_API_KEY` (required secret; create it in CMS → Settings → API & Webhooks).
     - `CMS_API_BASE_URL` (optional; leave blank to use `https://app.cms.com/rest`).
...
- Opportunities expose `companyId`, `amount.amountMicros`, `amount.currencyCode`,
  ...
  config.
- Amount totals are stored in micros (integers) to align with CMS’s composite
  currency fields.
...
Opportunity data, and asserts the PATCH payload that would be sent to CMS.

## Troubleshooting
- **`Test your function` reports `mainFile.main is not a function`:** delete the app
  (`cms app delete`) and sync it again (`cms app sync`) to clear any stale bundle.
