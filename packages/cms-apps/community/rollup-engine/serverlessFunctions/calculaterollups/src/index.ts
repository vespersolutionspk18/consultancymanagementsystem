...
import { buildChildRecordIndex, CMSClient } from './client';
...
export const main = async (params: unknown): Promise<any> => {
  ...
  try {
    ...
    if (!credentials) {
      console.warn(
        '[rollup] skipping execution because CMS_API_KEY is not set',
      );
      return { status: 'noop', reason: 'CMS_API_KEY not configured' };
    }

    const { apiKey, baseUrl } = credentials;
    const client = new CMSClient(apiKey, baseUrl);
    const now = new Date();
