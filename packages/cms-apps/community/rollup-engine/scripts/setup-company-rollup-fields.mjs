...
const envCandidates = [
  process.env.CMS_ENV_PATH ? path.resolve(process.env.CMS_ENV_PATH) : null,
...
envCandidates.forEach((candidate) => {
  ...
  if (!result.error && process.env.CMS_API_KEY) {
    return;
  }
});

const METADATA_BASE_URL = (
  process.env.CMS_METADATA_BASE_URL ??
  (process.env.CMS_API_BASE_URL
    ? `${process.env.CMS_API_BASE_URL.replace(/\/+$/, '')}/metadata`
    : null) ??
...
if (!API_KEY) {
  console.error('CMS_API_KEY is required in the environment to call the Metadata API.');
  process.exit(1);
}
