 ...
 * Run this after: npx cms-cli app sync packages/cms-apps/fireflies
 *
...
if (!API_KEY) {
  console.error('❌ Error: CMS_API_KEY not found in .env file');
  process.exit(1);
}
...
const findMeetingObject = async () => {
  ...
  if (!meetingEdge) {
    throw new Error('Meeting object not found. Please run "npx cms-cli app sync" first.');
  }
