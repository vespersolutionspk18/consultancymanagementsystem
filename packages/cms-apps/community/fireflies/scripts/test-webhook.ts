...
/**
 * Test script for Fireflies webhook against local CMS instance
 *
 * Usage:
 *   yarn test:webhook
 *   # or
 ...
 *
 * Prerequisites:
 *   1. CMS server running on http://localhost:3000
 *   2. Fireflies app synced: npx cms-cli app sync
...
const _mockFirefliesFetch = async (url: string, options?: RequestInit) => {
  ...
  // For CMS API calls, use real fetch
  return fetch(url, options);
};

const main = async () => {
  console.log('🧪 Testing Fireflies Webhook Against Local CMS Instance\n');
  console.log(`📍 Server URL: ${SERVER_URL}`);
  console.log(`🔑 API Key: ${CMS_API_KEY ? '✅ Configured' : '❌ Missing'}`);
  console.log(`🔐 Webhook Secret: ${FIREFLIES_WEBHOOK_SECRET ? '✅ Configured' : '⚠️  Using test secret'}\n`);

  // Validation
  if (!CMS_API_KEY) {
    console.error('❌ Error: CMS_API_KEY is required');
  ...
  try {
    ...
    console.error('   Make sure CMS is running: cd cms && yarn dev');
    process.exit(1);
  }
  ...
  if (signatureErrors) {
    ...
    if (response.ok) {
      ...
      console.log('\n📋 Next steps:');
      console.log('   1. Check CMS CRM for new Meeting/Note records');
    ...
    console.error('\n💡 Troubleshooting:');
    console.error('   1. Ensure CMS server is running');
    console.error('   2. Ensure app is synced: npx cms-cli app sync');
