...
Automatically captures meeting notes with AI-generated summaries and insights from Fireflies.ai into your CMS CRM.

### Current Status
- Doesn't work with Fireflies webhook yet due to missing headers forwarding in cms serverless func
...
**Fireflies webhook → Fireflies API → CMS CRM with summary-focused insights**
...
```bash
# Step 1: Authenticate with CMS
npx cms-cli auth login

# Step 2: Sync the app to create Meeting object
npx cms-cli app sync packages/cms-apps/fireflies
...
CMS_API_KEY=your_api_key             # From CMS CRM settings
SERVER_URL=https://your-domain.cms.com
```
...
```
https://your-cms-instance.com/s/webhook/fireflies
```
...
- CMS verifies signature using your webhook secret
- Invalid signatures are rejected immediately

### Current Platform Limitation (Headers)

- CMS serverless route triggers currently do **not forward HTTP headers** to functions. Fireflies signatures sent in headers are stripped, so header-based verification does not work in production.
...
- Ingest a specific Fireflies meeting into CMS:
`yarn meeting:ingest <meetingId>` or `MEETING_ID=... yarn meeting:ingest`

- Fetch all/historical Fireflies meetings into CMS:
`yarn meeting:all [--from 2024-01-01] [--to 2024-02-01] [--organizer a@x.com] [--participant b@x.com] [--channel <channelId>] [--mine] [--dry-run]`
  ...
  - Controls:
    - `--dry-run`: list and transform without writing to CMS
...
**Integration**: Fireflies webhook → AI processing layer → Enhanced CMS records
