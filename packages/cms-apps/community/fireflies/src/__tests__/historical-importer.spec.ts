...
describe('HistoricalImporter', () => {
  const buildImporter = () => {
    ...
    return { firefliesClient, cmsService };
  };

  it('skips meetings that already exist by firefliesMeetingId', async () => {
    const { firefliesClient, cmsService } = buildImporter();

    firefliesClient.listTranscripts.mockResolvedValue([{ id: 'existing' }]);
    cmsService.findMeetingByFirefliesId.mockResolvedValue({ id: 'cms-id' });

    const importer = new HistoricalImporter(firefliesClient, cmsService);
    ...
    expect(cmsService.createMeeting).not.toHaveBeenCalled();
    expect(result.statuses[0].status).toBe('skipped_existing');
  });

  it('supports dry-run without writing to CMS', async () => {
    const { firefliesClient, cmsService } = buildImporter();
    ...
    cmsService.findMeetingByFirefliesId.mockResolvedValue(undefined);
    ...
    expect(cmsService.createMeeting).not.toHaveBeenCalled();
    expect(cmsService.createNoteOnly).not.toHaveBeenCalled();
