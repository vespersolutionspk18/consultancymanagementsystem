...
import { CMSCrmService } from './cms-crm-service';
...
export class WebhookHandler {
  ...
  async handle(params: unknown, headers?: Record<string, string>): Promise<ProcessResult> {
    ...
    try {
      ...
      if (!cmsApiKey) {
        this.logger.critical('CMS_API_KEY not configured - this is a critical configuration error');
      ...
      const cmsService = new CMSCrmService(
        cmsApiKey,
        getApiUrl()
      );

      const existingMeetingById = await cmsService.findMeetingByFirefliesId(meetingData.id);
      ...
      const existingMeetingByTitle = await cmsService.findExistingMeeting(meetingData.title);
      ...
      const { matchedContacts, unmatchedParticipants } = await cmsService.matchParticipantsToContacts(
      ...
      const newContactIds = autoCreate
        ? await cmsService.createContactsForUnmatched(unmatchedParticipants)
        : [];
      ...
      const noteId = await cmsService.createNoteOnly(
      ...
      result.meetingId = await cmsService.createMeeting(meetingInput);
  ...
  private async linkNoteToParticipants(
    cmsService: CMSCrmService,
    noteId: string,
    contactIds: string[]
  ): Promise<void> {
    for (const contactId of contactIds) {
      try {
        await cmsService.createNoteTarget(noteId, contactId);
  ...
  private async createFailedMeetingRecord(params: unknown, error: string): Promise<void> {
    try {
      const cmsApiKey = process.env.CMS_API_KEY || '';
      if (!cmsApiKey) {
        this.logger.debug('Cannot create failed meeting record: CMS_API_KEY not configured');
        return;
      }
      ...
      const cmsService = new CMSCrmService(cmsApiKey, getApiUrl());
      ...
      const failedMeetingId = await cmsService.createFailedMeeting(failedMeetingData);
