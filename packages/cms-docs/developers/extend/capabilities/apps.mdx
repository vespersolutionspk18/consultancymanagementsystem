---
title: CMS Apps
description: Build and manage CMS customizations as code.
---
...
Apps let you build and manage CMS customizations **as code**. Instead of configuring everything through the UI, you define your data model and serverless functions in code — making it faster to build, maintain, and roll out to multiple workspaces.
...
See also: the CLI reference pages for [create-cms-app](https://www.npmjs.com/package/create-cms-app) and [cms-sdk CLI](https://www.npmjs.com/package/cms-sdk).

## Project structure (scaffolded)

When you run `npx create-cms-app@latest my-cms-app`, the scaffolder:

- Copies a minimal base application into `my-cms-app/`
- Adds a local `cms-sdk` dependency and Yarn 4 configuration
- Creates config files and scripts wired to the `cms` CLI
- Generates a default application config and a default function role

A freshly scaffolded app looks like this:

```text filename="my-cms-app/"
my-cms-app/
  package.json
  yarn.lock
  .gitignore
...
- **package.json**: Declares the app name, version, engines (Node 24+, Yarn 4), and adds `cms-sdk` plus scripts like `dev`, `sync`, `generate`, `create-entity`, `logs`, `uninstall`, and `auth` that delegate to the local `cms` CLI.
...
- `yarn generate` will create a `generated/` folder (typed CMS client + workspace types).
...
Your credentials are stored per-user in `~/.cms/config.json`. You can maintain multiple profiles and switch using `--workspace <name>`.
...
The cms-sdk provides typed building blocks you use inside your app. Below are the key pieces you'll touch most often.

### Defining objects

Custom objects are regular TypeScript classes annotated with decorators from `cms-sdk`. They live under `src/objects/` in your app and describe both schema and behavior for records in your workspace.
...
const config: ApplicationConfig = {
  ...
  displayName: 'My CMS App',
  description: 'My first CMS app',
...
- The runtime API key injected as `CMS_API_KEY` is derived from this default function role.
...
export const functionRole: RoleConfig = {
  ...
  label: 'My CMS App default function role',
  description: 'My CMS App default function role',
...
- See a working example in the Hello World app: [`packages/cms-apps/hello-world/src/roles/function-role.ts`](https://github.com/cmshq/cms/blob/main/packages/cms-apps/hello-world/src/roles/function-role.ts).
...
export const main = async (
  ...
  const client = new CMS(); // generated typed client
...
When your function runs on CMS, the platform injects credentials as environment variables before your code executes:

- `CMS_API_URL`: Base URL of the CMS API your app targets.
- `CMS_API_KEY`: Short‑lived key scoped to your application’s default function role.

Notes:
- You do not need to pass URL or API key to the generated client. It reads `CMS_API_URL` and `CMS_API_KEY` from process.env at runtime.
...
Explore a minimal, end-to-end example that demonstrates objects, functions, and multiple triggers [here](https://github.com/cmshq/cms/tree/main/packages/cms-apps/hello-world):

## Manual setup (without the scaffolder)

While we recommend using `create-cms-app` for the best getting-started experience, you can also set up a project manually. Do not install the CLI globally. Instead, add `cms-sdk` as a local dependency and wire scripts in your package.json:
...
- Cannot connect to server: verify the API URL and that the CMS server is reachable.
