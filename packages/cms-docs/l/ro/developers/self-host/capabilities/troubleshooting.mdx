...
Dacă aveți deja o instanță CMS cu date de producție, **NU** urmați acești pași deoarece vor șterge permanent baza dvs. de date!

În timp ce instalați CMS pentru prima dată, este posibil să doriți să schimbați parola implicită a bazei de date.
...
În timpul instalării CMS, trebuie să aprovizionați baza de date postgres cu schemele, extensiile și utilizatorii corecți.
...
#### Nu se poate găsi modulul 'cms-emails' sau declarațiile de tip corespunzătoare.

Trebuie să construiți pachetul `cms-emails` înainte de a rula inițializarea bazei de date cu `npx nx run cms-emails:build`

#### Pachetul cms-x lipsă

Asigurați-vă că rulați yarn în directorul rădăcină și apoi rulați `npx nx server:dev cms-server`. Dacă acest lucru nu funcționează, încercați să construiți manual pachetul lipsă.
...
#### În timpul rulării `npx nx start` sau `npx nx start cms-front`, apare o eroare Out of memory

În `packages/cms-front/.env` decomentați `VITE_DISABLE_TYPESCRIPT_CHECKER=true` și `VITE_DISABLE_ESLINT_CHECKER=true` pentru a dezactiva verificările de fundal, astfel reducând cantitatea de RAM necesară.

**If it does not work:**
Run only the services you need, instead of `npx nx start`. De exemplu, dacă lucrați la server, rulați doar `npx nx worker cms-server`
...
**If it does not work:**
Investigate which processes are taking you most of your machine RAM. La CMS, am observat că unele extensii VScode consumau multă RAM, așa că le dezactivăm temporar.
...
* Deschideți aplicația CMS, navigați la `/settings` și activați comutatorul **Avansat** în colțul din stânga jos al ecranului.
* Creați un nou webhook.
* Lipiți **Your Unique Webhook URL** în câmpul **Endpoint Url** din CMS. Setați **Filtrele** pe `Companii` și `Create`.
...
Dacă rulați CMS în spatele unui proxy invers și întâmpinați probleme de conexiune:
...
2. **Verificați Setările Proxy-ului Invers:**
   ...
   * Confirmați că proxy-ul invers transmite corect cererile către serverul CMS.
   * Asigurați-vă că headerele precum `X-Forwarded-For` și `X-Forwarded-Proto` sunt setate corect.

3. **Reporniți Serviciile:**

   După ce faceți schimbări, reporniți atât proxy-ul invers, cât și containerele CMS.
...
* Suport Comunitar:
  ...
  Contactați [comunitatea CMS](https://github.com/cmshq/cms/issues) sau [canalele de suport](https://discord.gg/cx5n4Jzs57) pentru asistență.
