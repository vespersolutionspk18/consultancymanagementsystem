...
```bash
npx nx run cms-front:lint # utilizați --fix pentru a corecta erorile de lint
```
...
```bash
npx nx run cms-front:test # rulați teste jest
npx nx run cms-front:storybook:serve:dev # rulați storybook
npx nx run cms-front:storybook:test # rulați testele # (are nevoie de yarn storybook:serve:dev pentru a rula)
npx nx run cms-front:storybook:coverage # (are nevoie de yarn storybook:serve:dev pentru a rula)
```
