...
Aceste comenzi ar trebui să fie executate din dosarul packages/cms-server.
...
```
npx nx database:reset cms-server # configurează baza de date cu seed-uri de dezvoltare
```
...
```
npx nx run cms-server:lint # adaugă --fix pentru a remedia erorile de lint
```

### Test

```
npx nx run cms-server:test:unit          # rulează teste unitare
npx nx run cms-server:test:integration   # rulează teste de integrare
```

Notă: poți rula `npx nx run cms-server:test:integration:with-db-reset` în cazul în care trebuie să resetezi baza de date înainte de a rula testele de integrare.
...
CMS folosește în principal NestJS pentru backend.
