...
description: Ghidul pentru contribuitori (sau dezvoltatori curioși) care doresc să ruleze CMS local.
---
...
<Tabs>
  <Tab title="Linux și MacOS">
    Înainte de a instala și utiliza CMS, asigurați-vă că instalați următoarele pe computerul dvs.:
...
CMS necesită un cache Redis pentru a oferi cea mai bună performanță
...
Pentru a construi serverul CMS și a adăuga date în baza dvs. de date, rulați următoarea comandă:
...
## Pasul 8: Utilizați CMS

**Front-end**

Frontend-ul CMS va rula la [http://localhost:3001](http://localhost:3001).
Vă puteți loga folosind contul demo implicit: `tim@apple.dev` (parolă: `tim@apple.dev`)

**Back-end**

* Serverul CMS va fi operativ la [http://localhost:3000](http://localhost:3000)
