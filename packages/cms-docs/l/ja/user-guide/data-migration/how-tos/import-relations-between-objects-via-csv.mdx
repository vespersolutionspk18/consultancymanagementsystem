...
The parent ("one" side) must exist in CMS before you can reference it.
...
1. Export the parent records from CMS
...
<Warning>
  **Domain format matters!**
  ...
  Use `https://domain.com` (not just `domain.com`). This matches how CMS stores Company domains and prevents matching errors.
</Warning>
...
CMS will automatically link each child record to the matching parent.
...
| **Non-existent records**   | The parent record doesn't exist in CMS          | Import parent records first, or check for typos         |
| **Case mismatch**          | `Acme.com` in file but `acme.com` in CMS        | Ensure exact case matching                              |
...
<AccordionGroup>
  <Accordion title="What if my parent record doesn't have a domain or email?">
    ...
    1. Use the CMS `id` (export parent records to get their IDs)
