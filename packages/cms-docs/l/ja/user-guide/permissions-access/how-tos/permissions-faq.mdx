...
<AccordionGroup>
  <Accordion title="What roles come with CMS by default?">
    CMS comes with an **Admin** and **Member** roles by default. You can create additional custom roles based on your team's needs (e.g., Sales Rep, Manager, Read-Only User).
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="What action permissions are available?">
    ...
    * **Send Email**: Ability to send emails from CMS
...
<AccordionGroup>
  ...
  <Accordion title="What is Just-in-Time (JIT) provisioning?">
    With JIT provisioning, user accounts are automatically created in CMS when someone logs in via SSO for the first time. They're assigned the default role automatically.
  </Accordion>
