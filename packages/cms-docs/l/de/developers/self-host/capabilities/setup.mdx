...
<Warning>
  **Erstinstallation?** Folgen Sie dem [Docker Compose-Installationshandbuch](/l/de/developers/self-host/capabilities/docker-compose), um CMS zum Laufen zu bringen, und kehren Sie dann hierher zurück, um die Konfiguration fortzusetzen.
</Warning>

CMS bietet **zwei Konfigurationsmodi**, um unterschiedlichen Implementierungsbedürfnissen gerecht zu werden:
...
1. Greifen Sie auf Ihre CMS-Instanz zu (normalerweise `http://localhost:3000`)
...
<Warning>
  ...
  [Vollständige technische Referenz →](https://github.com/cmshq/cms/blob/main/packages/cms-server/src/engine/core-modules/cms-config/config-variables.ts)
</Warning>
...
By default, CMS runs in **single-workspace mode** — ideal for most self-hosted deployments where you need one CRM instance for your organization.
...
* One workspace per CMS instance
...
Enable multi-workspace mode for SaaS-like deployments where multiple independent teams need their own workspaces on the same CMS instance.
...
[Siehe relevanten Quellcode](https://github.com/cmshq/cms/blob/main/packages/cms-server/src/engine/core-modules/auth/utils/get-google-apis-oauth-scopes.ts#L4-L10)
...
<Warning>
  Benutzer müssen eine [Microsoft 365-Lizenz](https://admin.microsoft.com/Adminportal/Home) besitzen, um die Kalender- und Messaging-API verwenden zu können. Ohne eine solche Lizenz können sie ihr Konto nicht mit CMS synchronisieren.
</Warning>
...
[Siehe relevanten Quellcode](https://github.com/cmshq/cms/blob/main/packages/cms-server/src/engine/core-modules/auth/utils/get-microsoft-apis-oauth-scopes.ts#L2-L9)
