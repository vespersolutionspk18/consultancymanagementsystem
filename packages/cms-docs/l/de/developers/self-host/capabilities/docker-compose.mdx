...
This guide provides step-by-step instructions to install and configure the CMS application using Docker Compose. Das Ziel ist, den Prozess übersichtlich zu gestalten und häufige Fallstricke zu vermeiden, die Ihre Einrichtung unterbrechen könnten.
...
Installieren Sie die neueste stabile Version von CMS mit einem einzigen Befehl:
...
If you host cmsCRM on your own computer, open your browser and navigate to [http://localhost:3000](http://localhost:3000).
...
### CMS für externen Zugriff freigeben

Standardmäßig läuft CMS auf `localhost` an Port `3000`. Um über eine externe Domain oder IP-Adresse darauf zuzugreifen, müssen Sie die `SERVER_URL` in Ihrer `.env`-Datei konfigurieren.
...
SSL (HTTPS) ist erforderlich, damit bestimmte Browserfunktionen ordnungsgemäß funktionieren. Obwohl diese Funktionen möglicherweise während der lokalen Entwicklung funktionieren (da Browser localhost anders behandeln), ist eine ordnungsgemäße SSL-Einrichtung erforderlich, wenn CMS auf einer regulären Domain gehostet wird.
...
Wir empfehlen dringend, CMS hinter einem Reverse-Proxy mit SSL-Beendigung für optimale Sicherheit und Funktionalität zu konfigurieren.
