...
Wenn Sie bereits eine bestehende CMS-Instanz mit Produktionsdaten haben, **NICHT** diesen Schritten folgen, da diese permanent Ihre Datenbank löschen!

Bei der ersten Installation von CMS möchten Sie möglicherweise das Standard-Datenbankpasswort ändern.
...
Während der CMS-Installation müssen Sie Ihre Postgres-Datenbank mit den richtigen Schemata, Erweiterungen und Benutzern bereitstellen.
...
#### Modul 'cms-emails' oder seine entsprechenden Typdeklarationen nicht gefunden.

Sie müssen das Paket `cms-emails` erstellen, bevor Sie die Initialisierung der Datenbank mit `npx nx run cms-emails:build` ausführen.

#### Fehlendes CMS-x Paket

Stellen Sie sicher, dass Sie `yarn` im Root-Verzeichnis ausführen und dann `npx nx server:dev cms-server` ausführen. Wenn das immer noch nicht funktioniert, versuchen Sie, das fehlende Paket manuell zu erstellen.
...
#### Beim Ausführen von `npx nx start` oder `npx nx start cms-front` wird ein Speicherfehler angezeigt

Kommentieren Sie in `packages/cms-front/.env` `VITE_DISABLE_TYPESCRIPT_CHECKER=true` und `VITE_DISABLE_ESLINT_CHECKER=true` aus, um Hintergrundprüfungen zu deaktivieren und den RAM-Bedarf zu reduzieren.

**If it does not work:**
Run only the services you need, instead of `npx nx start`. Wenn Sie zum Beispiel am Server arbeiten, führen Sie nur `npx nx worker cms-server` aus.
...
**If it does not work:**
Investigate which processes are taking you most of your machine RAM. Bei CMS haben wir festgestellt, dass einige vscode-Erweiterungen viel RAM verwenden, daher deaktivieren wir sie vorübergehend.
...
* Öffnen Sie Ihre CMS-App, navigieren Sie zu `/settings`, und aktivieren Sie den **Erweitert**-Schalter unten links auf dem Bildschirm.
* Erstellen Sie einen neuen Webhook.
* Fügen Sie **Ihre einzigartige Webhook-URL** in das Feld **Endpoint-Url** in CMS ein. Stellen Sie die **Filter** auf `Companies` und `Created` ein.
...
Kommentieren Sie das Checker-Plugin in `packages/cms-ui/vite-config.ts` wie im folgenden Beispiel aus
...
Wenn Sie CMS hinter einem Reverse-Proxy ausführen und Verbindungsprobleme haben:
...
2. **Reverse-Proxy-Einstellungen prüfen:**
   ...
   * Stellen Sie sicher, dass Ihr Reverse-Proxy Anfragen korrekt an den CMS-Server weiterleitet.
   * Stellen Sie sicher, dass Header wie `X-Forwarded-For` und `X-Forwarded-Proto` korrekt gesetzt sind.

3. **Dienste neu starten:**

   Starten Sie nach Änderungen sowohl den Reverse-Proxy als auch die CMS-Container neu.
...
* Unterstützung aus der Community:
  ...
  Wenden Sie sich an die [CMS Community](https://github.com/cmshq/cms/issues) oder [Support-Kanäle](https://discord.gg/cx5n4Jzs57) für Unterstützung.
