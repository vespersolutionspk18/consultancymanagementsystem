...
CMS wurde so entwickelt, dass es entwicklerfreundlich ist und leistungsstarke APIs bietet, die sich an Ihr individuelles Datenmodell anpassen. Wir bieten vier verschiedene API-Typen, um unterschiedlichen Integrationsanforderungen gerecht zu werden.

## Entwickler-Erst-Ansatz

CMS generates APIs specifically for your data model:
...
<Note>
  Your personalized API documentation is available under **Settings → API & Webhooks** after creating an API key. Since CMS generates APIs that match your custom data model, the documentation is unique to your workspace.
</Note>
