...
<Warning>
  **Prima volta che installi?** Segui la [guida all'installazione di Docker Compose](/l/it/developers/self-host/capabilities/docker-compose) per far funzionare CMS, quindi torna qui per la configurazione.
</Warning>

CMS offre **due modalità di configurazione** per soddisfare diverse esigenze di distribuzione:
...
1. Accedi alla tua istanza CMS (solitamente `http://localhost:3000`)
...
<Warning>
  ...
  [Riferimento tecnico completo →](https://github.com/cmshq/cms/blob/main/packages/cms-server/src/engine/core-modules/cms-config/config-variables.ts)
</Warning>
...
**Scope richiesti** (configurati automaticamente): [Vedi il codice sorgente pertinente](https://github.com/cmshq/cms/blob/main/packages/cms-server/src/engine/core-modules/auth/utils/get-google-apis-oauth-scopes.ts#L4-L10)
...
<Warning>
  Gli utenti devono avere una [Licenza Microsoft 365](https://admin.microsoft.com/Adminportal/Home) per poter utilizzare l'API Calendario e Messaggi. Non potranno sincronizzare il loro account su CMS senza una.
</Warning>
...
[Vedi il codice sorgente pertinente](https://github.com/cmshq/cms/blob/main/packages/cms-server/src/engine/core-modules/auth/utils/get-microsoft-apis-oauth-scopes.ts#L2-L9)
