...
```bash
npx nx run cms-front:lint # aggiungere --fix per correggere gli errori di lint
```
...
```bash
npx nx run cms-front:test # eseguire i test jest
npx nx run cms-front:storybook:serve:dev # eseguire storybook
npx nx run cms-front:storybook:test # eseguire i test # (è necessario che yarn storybook:serve:dev sia in esecuzione)
npx nx run cms-front:storybook:coverage # (è necessario che yarn storybook:serve:dev sia in esecuzione)
```
