...
description: La guida per i collaboratori (o sviluppatori curiosi) che vogliono eseguire CMS localmente.
---
...
<Tabs>
  <Tab title="Linux e MacOS">
    Prima di poter installare e usare CMS, assicurati di installare quanto segue sul tuo computer:
...
CMS richiede una cache Redis per offrire le migliori prestazioni
...
Per costruire il server CMS e popolare alcuni dati nel tuo database, esegui il seguente comando:
...
## Passo 8: Utilizza CMS

**Frontend**

Il frontend di CMS sarà in esecuzione su [http://localhost:3001](http://localhost:3001).
Puoi accedere utilizzando l'account demo predefinito: `tim@apple.dev` (password: `tim@apple.dev`)

**Backend**

* Il server di CMS sarà attivo e funzionante su [http://localhost:3000](http://localhost:3000)
