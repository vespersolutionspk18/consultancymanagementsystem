...
This guide provides step-by-step instructions to install and configure the CMS application using Docker Compose. The aim is to make the process straightforward and prevent common pitfalls that could break your setup.
...
Tek bir komutla en son kararlı CMS sürümünü yükleyin:
...
```bash
VERSION=vx.y.z BRANCH=dal-adı bash <(curl -sL https://raw.githubusercontent.com/cmshq/cms/main/packages/cms-docker/scripts/install.sh)
```
...
### CMS'yi Dış Erişime Açma

Varsayılan olarak, CMS `localhost` üzerinde `3000` portunda çalışır. Harici bir alan adı veya IP adresi aracılığıyla erişim sağlamak için `.env` dosyanızda `SERVER_URL`'i yapılandırmanız gerekir.
...
Belirli tarayıcı özelliklerinin düzgün çalışabilmesi için SSL (HTTPS) gereklidir. Bu özellikler yerel geliştirme sırasında çalışabilirken (tarayıcılar localhost'u farklı şekilde ele alır), CMS'yi düzenli bir alanda barındırırken uygun bir SSL kurulumu gereklidir.
...
Optimum güvenlik ve işlevsellik için CMS'nin SSL sonlandırma ile bir ters proxy arkasında ayarlanmasını şiddetle öneririz.
