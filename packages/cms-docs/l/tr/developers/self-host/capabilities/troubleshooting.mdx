...
Mevcut bir CMS örneğiniz varsa ve üretim verileri varsa, bu adımları **TAKİP ETMEYİN**, çünkü bu veritabanınızı kalıcı olarak silecektir!

CMS'yi ilk kez kurarken, varsayılan veritabanı parolasını değiştirmek isteyebilirsiniz.
...
CMS kurulumu sırasında, postgres veritabanınızı doğru şemalar, uzantılar ve kullanıcılarla hazırlamanız gerekir.
...
#### Modül 'cms-emails' veya buna karşılık gelen tür açıklamaları bulunamıyor.

`cms-emails` paketini veritabanını `npx nx run cms-emails:build` ile başlatmadan önce derlemeniz gerekiyor.

#### Eksik cms-x paketi

Kök dizinde yarn çalıştırdığınızdan ve ardından `npx nx server:dev cms-server` çalıştırdığınızdan emin olun. Bu hala çalışmıyorsa eksik paketi manuel olarak derlemeyi deneyin.
...
#### `npx nx start` veya `npx nx start cms-front` çalıştırırken, bellek hatası verildi

`packages/cms-front/.env` içinde `VITE_DISABLE_TYPESCRIPT_CHECKER=true` ve `VITE_DISABLE_ESLINT_CHECKER=true`'i açarak arka plan kontrollerini devre dışı bırakın, böylece gerekli RAM miktarını azaltın.

**If it does not work:**
Run only the services you need, instead of `npx nx start`. Örneğin, sunucuda çalışıyorsanız yalnızca `npx nx worker cms-server` çalıştırın.
...
**If it does not work:**
Investigate which processes are taking you most of your machine RAM. CMS'de bazı VScode uzantılarının çok fazla RAM aldığını fark ettik, bu yüzden onları geçici olarak devre dışı bırakıyoruz.
...
* CMS uygulamanızı açın, `/settings` sayfasına gidin ve ekranın sol alt köşesinde bulunan **Gelişmiş** seçeneğini etkinleştirin.
* Yeni bir webhook oluşturun.
* **Benzersiz Webhook URL'nizi** CMS'deki **Endpoint URL** alanına yapıştırın. **Filtreleri** `Şirketler` ve `Oluşturuldu` olarak ayarlayın.
...
Eğer CMS'yi ters proxy'nin arkasında çalıştırıyorsanız ve bağlantı sorunları yaşıyorsanız:
...
2. **Ters Proxy Ayarlarını Kontrol Edin:**
   ...
   * Ters proxy'nizin CMS sunucusuna talepleri düzgün bir şekilde yönlendirdiğinden emin olun.
...
3. **Servisleri Yeniden Başlatın:**
   ...
   Değişiklik yaptıktan sonra hem ters proxy'yi hem de CMS konteynerlerini yeniden başlatın.
...
* Topluluk Desteği:
  ...
  [CMS topluluğuna](https://github.com/cmshq/cms/issues) veya [destek kanallarına](https://discord.gg/cx5n4Jzs57) yardım için ulaşın.
