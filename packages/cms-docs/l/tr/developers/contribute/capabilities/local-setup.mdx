---
title: Yerel Kurulum
description: CMS'i yerel olarak çalıştırmak isteyen katkıda bulunanlar (veya meraklı geliştiriciler) için kılavuz.
---
...
<Tabs>
  <Tab title="Linux ve MacOS">
    CMS'i yüklemeden ve kullanmadan önce bilgisayarınıza aşağıdakileri yüklediğinizden emin olun:
...
CMS, en iyi performansı sağlamak için bir redis önbelleğe ihtiyaç duyar
...
CMS server'ı oluşturup veritabanınıza bazı veriler yerleştirmek için aşağıdaki komutu çalıştırın:
...
CMS's frontend will be running at [http://localhost:3001](http://localhost:3001).
Varsayılan demo hesabıyla giriş yapabilirsiniz: `tim@apple.dev` (şifre: `tim@apple.dev`)

**Arka Uç**

* CMS's server will be up and running at [http://localhost:3000](http://localhost:3000)
