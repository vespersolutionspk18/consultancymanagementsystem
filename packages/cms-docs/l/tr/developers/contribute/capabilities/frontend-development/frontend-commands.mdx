...
```bash
npx nx run cms-front:lint # lint hatalarını düzeltmek için --fix'i geç
```
...
```bash
npx nx run cms-front:test # jest testlerini çalıştır
npx nx run cms-front:storybook:serve:dev # storybook'u çalıştır
npx nx run cms-front:storybook:test # testleri çalıştır # (yarn storybook:serve:dev'nin çalışıyor olması gerekir)
npx nx run cms-front:storybook:coverage # (yarn storybook:serve:dev'nin çalışıyor olması gerekir)
```
