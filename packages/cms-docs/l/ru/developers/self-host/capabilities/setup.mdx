...
<Warning>
  **Устанавливаете впервые?** Следуйте [руководству по установке Docker Compose](/l/ru/developers/self-host/capabilities/docker-compose), чтобы запустить CMS, затем вернитесь сюда для настройки.
</Warning>

CMS предлагает **два режима конфигурации**, чтобы соответствовать разным потребностям развертывания:
...
1. Получите доступ к вашему экземпляру CMS (обычно `http://localhost:3000`)
...
<Warning>
  ...
  [Полная техническая ссылка →](https://github.com/cmshq/cms/blob/main/packages/cms-server/src/engine/core-modules/cms-config/config-variables.ts)
</Warning>
...
соответствующий исходный код](https://github.com/cmshq/cms/blob/main/packages/cms-server/src/engine/core-modules/auth/utils/get-google-apis-oauth-scopes.ts#L4-L10)
...
<Warning>
  Пользователи должны иметь [лицензию Microsoft 365](https://admin.microsoft.com/Adminportal/Home), чтобы иметь возможность использовать API Календаря и Сообщений. Они не смогут синхронизировать свою учетную запись в CMS без нее.
</Warning>
...
соответствующий исходный код](https://github.com/cmshq/cms/blob/main/packages/cms-server/src/engine/core-modules/auth/utils/get-microsoft-apis-oauth-scopes.ts#L2-L9)
