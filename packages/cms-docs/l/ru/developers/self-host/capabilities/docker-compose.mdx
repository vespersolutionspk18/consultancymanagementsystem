...
Это руководство предоставляет пошаговые инструкции по установке и настройке приложения CMS с помощью Docker Compose. Цель заключается в том, чтобы сделать процесс простым и избежать распространенных ошибок, которые могут привести к сбоям в настройке.
...
Установите последнюю стабильную версию CMS одной командой:

```bash
bash <(curl -sL https://raw.githubusercontent.com/cmshq/cms/main/packages/cms-docker/scripts/install.sh)
```

Для установки конкретной версии или ветки:

```bash
VERSION=vx.y.z BRANCH=branch-name bash <(curl -sL https://raw.githubusercontent.com/cmshq/cms/main/packages/cms-docker/scripts/install.sh)
```
...
1. **Создайте файл .env**
   ...
   ```bash
   curl -o .env https://raw.githubusercontent.com/cmshq/cms/refs/heads/main/packages/cms-docker/.env.example
   ```
...
```bash
curl -o docker-compose.yml https://raw.githubusercontent.com/cmshq/cms/refs/heads/main/packages/cms-docker/docker-compose.yml
```
...
Если вы размещаете cmsCRM на своем компьютере, откройте ваш браузер и перейдите на [http://localhost:3000](http://localhost:3000).
...
### Открыть доступ к CMS из внешней сети

По умолчанию CMS работает на `localhost` на порту `3000`. Чтобы получить доступ через внешний домен или IP-адрес, необходимо настроить `SERVER_URL` в вашем `.env` файле.
...
SSL (HTTPS) требуется для правильной работы некоторых функций браузера. Хотя эти функции могут работать во время разработки на локальном компьютере (поскольку браузеры обрабатывают localhost иначе), правильная настройка SSL необходима при размещении CMS на регулярном домене.
...
Мы настоятельно рекомендуем настроить CMS за обратным прокси-сервером с прекращением SSL для оптимальной безопасности и функциональности.
...
```bash
docker exec cms-postgres pg_dump -U postgres cms > backup_$(date +%Y%m%d).sql
```
...
```bash
0 2 * * * docker exec cms-postgres pg_dump -U postgres cms > /backups/cms_$(date +\%Y\%m\%d).sql
```
...
```bash
docker compose stop cms-server cms-front
```

2. Restore the database:

```bash
docker exec -i cms-postgres psql -U postgres cms < backup_20240115.sql
```
