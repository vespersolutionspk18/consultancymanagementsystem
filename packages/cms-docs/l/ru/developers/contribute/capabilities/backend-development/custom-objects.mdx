...
Объекты - это структуры, которые позволяют хранить данные (записи, атрибуты и значения), специфичные для организации. CMS provides both standard and custom objects.

Стандартные объекты - это встроенные объекты с набором атрибутов, доступных для всех пользователей. Examples of standard objects in CMS include Company and Person. Стандартные объекты имеют стандартные поля, которые также доступны всем пользователям CMS, например, Company.displayName.
