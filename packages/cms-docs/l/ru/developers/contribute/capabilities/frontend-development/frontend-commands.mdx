...
```bash
npx nx run cms-front:lint # используйте --fix для исправления ошибок линтинга
```
...
```bash
npx nx run cms-front:test # запуск тестов jest
npx nx run cms-front:storybook:serve:dev # запуск storybook
npx nx run cms-front:storybook:test # запуск тестов # (требуется yarn storybook:serve:dev для работы)
npx nx run cms-front:storybook:coverage # (требуется yarn storybook:serve:dev для работы)
```
