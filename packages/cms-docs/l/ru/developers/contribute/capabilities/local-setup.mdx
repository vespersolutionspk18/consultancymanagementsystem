...
description: Руководство для участников (или любопытных разработчиков), которые хотят запускать CMS локально.
---
...
<Tabs>
  <Tab title="Linux и MacOS">
    Прежде чем установить и использовать CMS, убедитесь, что у вас установлено следующее:
...
CMS требует кэша Redis для обеспечения наилучшей производительности
...
Чтобы собрать сервер CMS и добавить данные в вашу базу данных, выполните следующую команду:
...
## Шаг 8: Использовать CMS

**Frontend**

Фронтенд CMS будет работать на [http://localhost:3001](http://localhost:3001).
Вы можете войти, используя учетную запись демо по умолчанию: `tim@apple.dev` (пароль: `tim@apple.dev`)

**Backend**

* Сервер CMS будет работать на [http://localhost:3000](http://localhost:3000).
