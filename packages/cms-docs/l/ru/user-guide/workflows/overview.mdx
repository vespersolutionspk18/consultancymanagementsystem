...
CMS был создан, чтобы обеспечить максимальную гибкость для своих пользователей. Rather than forcing you to adapt your business processes to rigid, pre-built features, workflows enable you to build automations that create the CRM that best supports your unique business use cases.

Workflows - это встроенная функция CMS для создания этих автоматизаций. Они дают вам строительные блоки, чтобы создать именно то, что нужно вашему бизнесу, именно тогда, когда это необходимо.
...
2. **Добавьте и извлеките данные из CMS**: Подключите CMS через API вызовы и вебхуки к вашей базе данных и другим инструментам.
