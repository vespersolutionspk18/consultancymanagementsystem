...
description: Step-by-step guide to migrate your data from any CRM to CMS.
---

## Přehled

This guide walks you through migrating your data from any CRM to CMS. The process involves auditing your data, preparing your CMS workspace, exporting from your current system, and importing into CMS.
...
Create a mapping document between your current CRM and CMS:
...
## Step 3: Set Up Your CMS Workspace

Before importing data, prepare your CMS workspace:
...
<Warning>
  **Invite users BEFORE importing data.**
  ...
  If your data includes user references (Account Owner, Assignee, etc.), those users must exist in CMS before import. Otherwise, those relations cannot be mapped.
</Warning>
...
Open each exported CSV in a spreadsheet application and prepare it for CMS.

### Remove Duplicates

1. Sort by the unique field (email for People, domain for Companies)
2. Remove or merge duplicate rows
3. Verify no duplicates exist in CMS already
...
<Warning>
  **Domain format is critical.**
  ...
  Use `https://domain.com` (not `domain.com` or `www.domain.com`). This matches CMS's format and prevents duplicates when you connect email/calendar sync.
</Warning>
...
2. Use the same email addresses that users used to join your CMS workspace
...
1. Navigate to the object in CMS
...
* CMS will start syncing emails to contact records
...
<AccordionGroup>
  <Accordion title="Can I migrate workflows automatically?">
    Not currently. Workflows must be recreated manually in CMS.
  </Accordion>
...
Contact [contact@cms.com](mailto:contact@cms.com) or explore our [Implementation Services](/l/cs/user-guide/getting-started/capabilities/implementation-services).
