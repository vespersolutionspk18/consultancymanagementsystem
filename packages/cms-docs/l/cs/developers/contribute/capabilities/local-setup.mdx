...
description: Průvodce pro přispěvatele (nebo zvídavé vývojáře), kteří chtějí spustit CMS lokálně.
---
...
<Tabs>
  <Tab title="Linux a MacOS">
    Než nainstalujete a použijete CMS, ujistěte se, že máte na svém počítači nainstalovány následující balíčky:
...
CMS vyžaduje ke svému provozu Redis cache pro zajištění nejlepšího výkonu.
...
Pro sestavení CMS serveru a zavedení některých dat do vaší databáze spusťte následující příkaz:
...
Frontend CMS bude spuštěný na [http://localhost:3001](http://localhost:3001).
Můžete se přihlásit pomocí výchozího demo účtu: `tim@apple.dev` (heslo: `tim@apple.dev`)

**Backend**

* Server CMS poběží na [http://localhost:3000](http://localhost:3000)
