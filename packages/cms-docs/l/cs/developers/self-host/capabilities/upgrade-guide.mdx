...
1. V terminálu, na hostiteli, kde CMS běží, vypněte CMS: `docker compose down`
...
3. Opětovně zapněte CMS pomocí `docker compose up -d`
...
Pokud po upgradu narazíte na problémy s výkonem, může být nutné vyprázdnit cache, aby bylo zajištěno její sladění s nejnovějšími změnami. Spusťte tento příkaz v kontejneru cms-server:
...
Upgradujte svou instanci CMS pro použití v0.55 image
...
Ve svém kontejneru `cms-server` spusťte:

```bash
yarn command:prod cache:flush
```

Tento problém je specifický pro tuto verzi CMS a neměl by být vyžadován pro budoucí upgrady.
...
Měla by se vždy nacházet v rozmezí vaší aktuální instance CMS `major.minor` verze. Můžete zkontrolovat verzi instance na ovládacím panelu administrátora (na `/settings/admin-panel`, přístupné, pokud máte v databázi nastaveno uživatelské vlastnosti `canAccessFullAdminPanel` na true) nebo spuštěním `echo $APP_VERSION` ve vašem kontejneru `cms-server`.

Chcete-li opravit asynchronizaci verzí pracovního prostoru, budete muset upgradovat z odpovídající verze cms podle souvisejícího průvodce upgradem po jednotlivých krocích, až do dosažení požadované verze.
...
Upgradujte svou instanci CMS pro použití v0.52 image
...
Budete muset ručně změnit verzi pracovního prostoru na `0.51.0` v databázi a upgradovat pomocí cms verze `0.52.11` podle jejího výše uvedeného průvodce upgradem.

### v0.50 až v0.51

Upgradujte svou instanci CMS pro použití v0.51 image
...
Upgradujte svou instanci CMS pro použití v0.50.0 image
...
Aktualizujte svůj místní `docker-compose.yml` pomocí [docker-compose.yml v0.50.0](https://github.com/cmshq/cms/blob/v0.50.0/packages/cms-docker/docker-compose.yml)

### v0.43.0 až v0.44.0

Upgradujte svou instanci CMS pro použití v0.44.0 image
...
Upgradujte svou instanci CMS pro použití v0.43.0 image
...
2. Upgradujte svůj `docker-compose.yml` na použití obrazu postgres:16 podle [docker-compose.yml](https://raw.githubusercontent.com/cmshq/cms/main/packages/cms-docker/docker-compose.yml) soubor.
...
Upgradujte svou instanci CMS pro použití v0.42.0 image
...
Upgradujte svou instanci CMS pro použití v0.41.0 image
...
Upgradujte svou instanci CMS pro použití v0.40.0 image
...
Upgradujte svou instanci CMS pro použití v0.35.0 image
...
Upgradujte svou instanci CMS pro použití v0.34.0 image
...
Upgradujte svou instanci CMS pro použití v0.33.0 image
...
Od této verze se obraz cms-postgres pro DB stal zastaralým a místo něj se používá cms-postgres-spilo.
Pokud chcete pokračovat v používání obrazu cms-postgres, jednoduše nahraďte `cmscrm/cms-postgres:${TAG}` za `cmscrm/cms-postgres` v docker-compose.yml.

### v0.31.0 až v0.32.0

Upgradujte svou instanci CMS pro použití v0.32.0 image
...
Upgradujte svou instanci CMS pro použití v0.31.0 image
...
Upgradujte svou instanci CMS pro použití v0.30.0 image

**Breaking change**:
To enhance performances, CMS now requires redis cache to be configured. Aktualizovali jsme náš [docker-compose.yml](https://raw.githubusercontent.com/cmshq/cms/main/packages/cms-docker/docker-compose.yml), aby to odrážel.
...
Upgradujte svou instanci CMS pro použití v0.24.0 image
...
Upgradujte svou instanci CMS pro použití v0.23.0 image
...
Upgradujte svou instanci CMS pro použití v0.22.0 image
