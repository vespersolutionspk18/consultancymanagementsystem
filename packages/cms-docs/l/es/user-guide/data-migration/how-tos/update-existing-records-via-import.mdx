...
When you import a file containing a **unique identifier** that matches an existing record, CMS updates that record instead of creating a duplicate.

| If unique identifier...    | CMS will...                                   |
| -------------------------- | ------------------------------------------------ |
...
4. **Don't change unique identifier values** — or CMS will create new records
...
<Warning>
  **Don't change the unique identifier values.**
  ...
  If you change `john@acme.com` to `john.smith@acme.com`, CMS will create a new record instead of updating the existing one.
</Warning>
...
CMS matches records by the unique identifier and updates them with new values.
...
3. CMS will match on that field
...
| **Case mismatch**              | File has `John@acme.com` but CMS has `john@acme.com` | Creates new record                     | Export from CMS to get exact values    |
...
<AccordionGroup>
  ...
  <Accordion title="How do I know which records will be updated vs. created?">
    During the import review step, CMS shows you how many records will be updated vs. created based on unique identifier matches.
  </Accordion>
