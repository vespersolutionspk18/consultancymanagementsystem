...
After uploading your file and mapping columns, CMS validates your data:

1. **Validation runs** — CMS checks each row for errors
...
| --------------------------------------------------------------------- | ------------------------------------------------------------ |
| Duplicate values highlighted in yellow                                | Value already exists in CMS or appears twice in your file |
...
**Option 3: Let CMS update the existing record**

1. Ensure your file includes a unique identifier (`email`, `domain`, or `id`)
2. Map the unique identifier field
3. CMS will update the existing record instead of creating a duplicate
...
<Note>
  ...
  * If you keep the `email` but change the `id` → the id will be updated
  ...
  As long as one unique identifier matches, CMS updates the record.
</Note>
...
3. Check if records already exist in CMS
...
This means there is no Company in CMS with that specific identifier.
...
You're trying to link to a record that doesn't exist in CMS.
...
A column in your file can't be mapped because the field doesn't exist in CMS.
...
1. **User hasn't accepted their invitation** — the user doesn't exist in CMS yet
2. **Using user ID from old system** — CMS can't match IDs from another system
3. **Wrong email format** — the email doesn't match the user's CMS account

### How to Fix

1. Ensure all users have **accepted their invitation** to your CMS workspace
2. Use the user's **email address** (not their name or old system ID)
3. Use the same email they used to join CMS
...
<Check>Domain format matches CMS's format exactly</Check>
...
1. **Download the sample file** — see the exact format CMS expects
