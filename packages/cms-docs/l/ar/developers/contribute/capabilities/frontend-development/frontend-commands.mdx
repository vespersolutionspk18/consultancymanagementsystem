...
```bash
"npx nx run cms-front:lint # مرر --fix لإصلاح أخطاء التدقيق"
```
...
```bash
"npx nx run cms-front:test # تشغيل اختبارات jest\nnpx nx run cms-front:storybook:serve:dev # تشغيل storybook\nnpx nx run cms-front:storybook:test # تشغيل الاختبارات # (يحتاج إلى تشغيل yarn storybook:serve:dev)\nnpx nx run cms-front:storybook:coverage # (يحتاج إلى تشغيل yarn storybook:serve:dev)"
```
