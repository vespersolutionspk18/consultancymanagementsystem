...
إذا كان لديك تطبيق CMS موجود يحتوي على بيانات إنتاج، **لا تتبع هذه الخطوات لأنها ستحذف قاعدة البيانات الخاصة بك بشكل دائم!**

أثناء تثبيت CMS لأول مرة، قد ترغب في تغيير كلمة المرور الافتراضية لقاعدة البيانات.
...
أثناء تثبيت CMS، تحتاج إلى توفير قاعدة بيانات بوستجريس الخاصة بك بالمخططات والإضافات والمستخدمين الصحيحة.
...
#### لا يمكن العثور على الوحدة النمطية 'cms-emails' أو إعلانات نوعها المقابلة.

عليك بناء حزمة `cms-emails` قبل تشغيل تهيئة قاعدة البيانات باستخدام `npx nx run cms-emails:build`.

#### Missing cms-x package

تأكد من تشغيل yarn في الدليل الجذر ثم تشغيل `npx nx server:dev cms-server`. إذا لم يعمل ذلك، حاول بناء الحزمة المفقودة يدوياً.
...
#### أثناء تشغيل `npx nx start` أو `npx nx start cms-front`، ظهرت خطأ نفاد الذاكرة

في `packages/cms-front/.env` قم بإزالة تعليق على `VITE_DISABLE_TYPESCRIPT_CHECKER=true` و`VITE_DISABLE_ESLINT_CHECKER=true` لتعطيل فحوصات الخلفية مما يقلل من كمية الذاكرة المطلوبة.

**If it does not work:**
Run only the services you need, instead of `npx nx start`. على سبيل المثال، إذا كنت تعمل على الخادم، قم بتشغيل `npx nx worker cms-server` فقط
...
**If it does not work:**
Investigate which processes are taking you most of your machine RAM. في CMS، لاحظنا أن بعض إضافات VScode كانت تستهلك الكثير من الذاكرة لذا قمنا بتعطيلها مؤقتًا.
...
* افتح تطبيق CMS الخاص بك، انتقل إلى `/settings`، وفعل التبديل المتقدم في الجزء السفلي الأيسر من الشاشة.
* إنشاء ويب هوك جديد.
...
قم بتعليق مكون التحليل في `packages/cms-ui/vite-config.ts` كما في المثال أدناه
...
إذا كنت تستخدم CMS خلف بروكسي عكسي وواجهت مشاكل في الاتصال:
...
2. **التحقق من إعدادات البروكسي العكسي:**
   ...
   * تأكد من أن البروكسي العكسي يقوم بتمرير الطلبات بشكل صحيح إلى خادم CMS.
   * تأكد من أن رؤوس مثل `X-Forwarded-For` و`X-Forwarded-Proto` تم ضبطها بشكل صحيح.

3. **إعادة تشغيل الخدمات:**

   بعد إجراء التغييرات، أعد تشغيل كل من البروكسي العكسي وحاويات CMS.
...
* الدعم المجتمعي:
  ...
  تواصل مع [مجتمع CMS](https://github.com/cmshq/cms/issues) أو [قنوات الدعم](https://discord.gg/cx5n4Jzs57) للحصول على المساعدة.
