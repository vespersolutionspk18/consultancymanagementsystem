...
import { IconDotsVertical } from 'cms-ui/display';
...
export const DefaultWithoutSearch: Story = {
  play: async () => {
    ...
    expect(await canvas.findByText('Go to Projects')).toBeVisible();
...
export const LimitedPermissions: Story = {
  play: async () => {
    const canvas = within(document.body);
    expect(await canvas.findByText('Go to People')).toBeVisible();
    expect(canvas.queryByText('Go to Projects')).not.toBeInTheDocument();
