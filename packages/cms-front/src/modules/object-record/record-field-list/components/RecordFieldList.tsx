...
import { RecordDetailRichTextSection } from '@/object-record/record-field-list/record-detail-section/rich-text/components/RecordDetailRichTextSection';
  ...
  return (
    ...
    >
      ...
      {boxedRelationFieldMetadataItems
        ...
        .flatMap((fieldMetadataItem, index) => {
          const baseFieldDefinition = formatFieldMetadataItemAsColumnDefinition(
            {
              field: fieldMetadataItem,
              position: index,
              objectMetadataItem,
          ...
          const fieldDefinition = relationFieldLabelOverrides[
            fieldMetadataItem.name
          ]
            ? {
                ...baseFieldDefinition,
                label:
                  relationFieldLabelOverrides[fieldMetadataItem.name] ??
          ...
          // Insert rich text fields after sectors for Project and Company
          if (
            fieldMetadataItem.name === 'sectors' &&
            richTextV2FieldMetadataItems.length > 0
          ) {
            const richTextElements = richTextV2FieldMetadataItems.map(
              (richTextField, rtIndex) => (
                <FieldContext.Provider
                  key={objectRecordId + richTextField.id}
                  value={{
                    recordId: objectRecordId,
                    isLabelIdentifier: false,
                    fieldDefinition: formatFieldMetadataItemAsColumnDefinition({
                    ...
                    isRecordFieldReadOnly: isRecordFieldReadOnly({
                      ...
                      fieldMetadataItem: {
                        id: richTextField.id,
                        isUIReadOnly: richTextField.isUIReadOnly ?? false,
                      },
                    }),
                  }}
                >
                  <RecordDetailRichTextSection
                    loading={isPrefetchLoading || recordLoading}
                  />
                </FieldContext.Provider>
              ),
            );
            return [relationElement, ...richTextElements];
          }
