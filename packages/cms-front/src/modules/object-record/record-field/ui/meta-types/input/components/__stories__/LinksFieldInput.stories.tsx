...
import { getCanvasElementForDropdownTesting } from 'cms-ui/testing';
  ...
  args: {
    value: {
      ...
      secondaryLinks: [
        ...
        {
          url: 'https://github.com/cmshq/cms',
          label: 'GitHub',
        },
      ],
    },
...
export const CreatePrimaryLink: Story = {
  play: async ({ canvasElement }) => {
    ...
    const input = await canvas.findByPlaceholderText('URL');
    await userEvent.type(input, 'https://www.cms.com{enter}');

    const linkDisplay = await canvas.findByText('cms.com');
...
export const AddSecondaryLink: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const input = await canvas.findByPlaceholderText('URL');
    await userEvent.type(input, 'https://docs.cms.com{enter}');

    const secondaryLink = await canvas.findByText('docs.cms.com');
...
export const DeletePrimaryLink: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const listItemToDelete = await canvas.findByText('CMS Website');
...
export const DeletePrimaryLinkAndUseSecondaryLinkAsTheNewPrimaryLink: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const oldPrimaryLink = canvas.queryByText('CMS Website');
