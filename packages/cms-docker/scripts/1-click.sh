pull_version=${VERSION:-$(curl -s https://api.github.com/repos/cmshq/cms/tags | grep '"name":' | head -n 1 | cut -d '"' -f 4)}
...
if [[ -n "$BRANCH" ]] || [[ "$(printf '%s\n' "$target_version" "$version_num" | sort -V | head -n1)" != "$version_num" ]]; then
  curl -sL "https://raw.githubusercontent.com/cmshq/cms/$pull_branch/packages/cms-docker/scripts/install.sh" -o cms_install.sh
else
  curl -sL "https://raw.githubusercontent.com/cmshq/cms/$pull_branch/install.sh" -o cms_install.sh
fi

chmod +x cms_install.sh
VERSION="$VERSION" BRANCH="$BRANCH" ./cms_install.sh
