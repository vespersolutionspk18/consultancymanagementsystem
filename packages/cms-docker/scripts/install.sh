...
version=${VERSION:-$(curl -s "https://hub.docker.com/v2/repositories/cmscrm/cms/tags" | grep -o '"name":"[^"]*"' | grep -v 'latest' | cut -d'"' -f4 | sort -V | tail -n1)}
branch=${BRANCH:-$(curl -s https://api.github.com/repos/cmshq/cms/tags | grep '"name":' | head -n 1 | cut -d '"' -f 4)}
...
# Copy cms/packages/cms-docker/docker-compose.yml in it
echo -e "\t• Copying docker-compose.yml"
curl -sLo docker-compose.yml https://raw.githubusercontent.com/cmshq/cms/$branch/packages/cms-docker/docker-compose.yml

# Copy cms/packages/cms-docker/.env.example to .env
echo -e "\t• Setting up .env file"
curl -sLo .env https://raw.githubusercontent.com/cmshq/cms/$branch/packages/cms-docker/.env.example
