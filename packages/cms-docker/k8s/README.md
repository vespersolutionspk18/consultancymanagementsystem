...
This repository contains Kubernetes manifests and Terraform files to help you deploy and manage the CMSCRM application. The files are located in the `packages/cms-docker/k8s` directory.
...
cd cmscrm/packages/cms-docker/k8s
```
...
2. Create Server Secret
    ...
    ``` bash
    kubectl create secret generic -n cmscrm tokens --from-literal accessToken=changeme --from-literal loginToken="changeme" --from-literal refreshToken="changeme" --from-literal fileToken="changeme"
    ```
