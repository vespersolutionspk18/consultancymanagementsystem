 ...
 *
 * Usage:
 *   CROWDIN_PERSONAL_TOKEN=xxx npx ts-node packages/cms-utils/translation-qa-report.ts
 *
 * The token can be obtained from: https://cms.crowdin.com/u/settings#api-key
 */

import * as fs from 'fs';
import * as path from 'path';

const CROWDIN_BASE_URL = 'https://cms.api.crowdin.com/api/v2';
...
async function getToken(): Promise<string> {
  ...
  if (!token) {
    ...
    console.error(
      'Get your token from: https://cms.crowdin.com/u/settings#api-key',
    );
    process.exit(1);
  }
...
---
**Fix in Crowdin**: https://cms.crowdin.com/u/projects/1/all?filter=qa-issue

*To auto-fix encoding issues, run:*
\`\`\`
CROWDIN_PERSONAL_TOKEN=xxx npx ts-node packages/cms-utils/fix-crowdin-translations.ts
\`\`\`
`;
