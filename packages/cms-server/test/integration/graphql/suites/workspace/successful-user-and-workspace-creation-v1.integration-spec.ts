...
describe('Successful user and workspace creation', () => {
  ...
  it('should sign up a new user and create a new workspace successfully', async () => {
    ...
    const cmsStandardApp = findManyApplicationsData.find(
      (application) =>
        application.universalIdentifier ===
        CMS_STANDARD_APPLICATION.universalIdentifier,
    );

    jestExpectToBeDefined(cmsStandardApp);
    const {
      sourcePath: _sourcePath,
      sourceType: _sourceType,
      ...expectedStandardCMSApplication
    } = CMS_STANDARD_APPLICATION;

    expect(cmsStandardApp).toMatchObject(expectedStandardCMSApplication);
