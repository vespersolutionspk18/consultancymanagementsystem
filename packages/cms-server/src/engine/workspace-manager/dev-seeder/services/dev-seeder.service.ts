...
import { CMSStandardApplicationService } from 'src/engine/workspace-manager/cms-standard-application/services/cms-standard-application.service';
...
export class DevSeederService {
  constructor(
    ...
    private readonly cmsStandardApplicationService: CMSStandardApplicationService,
  ...
  public async seedDev(workspaceId: SeededWorkspacesIds): Promise<void> {
    const isBillingEnabled = this.cmsConfigService.get('IS_BILLING_ENABLED');
    const appVersion = this.cmsConfigService.get('APP_VERSION');
    ...
    const { workspaceCustomFlatApplication, cmsStandardFlatApplication } =
    ...
    await this.cmsStandardApplicationService.synchronizeCMSStandardApplicationOrThrow(
      {
        workspaceId,
      },
    );
