...
import { Sources } from 'cms-shared/types';
import {
...
export class ServerlessFunctionService {
  ...
  async executeOneServerlessFunction({
    ...
    const baseUrl = cleanServerUrl(this.cmsConfigService.get('SERVER_URL'));
    ...
    if (this.cmsConfigService.get('SERVERLESS_LOGS_ENABLED')) {
  ...
  async publishOneServerlessFunctionOrFail(id: string, workspaceId: string) {
    ...
    // See https://github.com/cmshq/cms/issues/13058
  ...
  private async throttleExecution(workspaceId: string) {
    try {
      await this.throttlerService.tokenBucketThrottleOrThrow(
        ...
        1,
        this.cmsConfigService.get('SERVERLESS_FUNCTION_EXEC_THROTTLE_LIMIT'),
        this.cmsConfigService.get('SERVERLESS_FUNCTION_EXEC_THROTTLE_TTL'),
      );
    } catch {
      throw new ServerlessFunctionException(
