...
import { getUniqueConstraintsFields, isDefined } from 'cms-shared/utils';
...
import { type ConnectObject } from 'src/engine/cms-orm/entity-manager/types/query-deep-partial-entity-with-nested-relation-fields.type';
import {
  type RelationConnectQueryConfig,
  type UniqueConstraintCondition,
} from 'src/engine/cms-orm/entity-manager/types/relation-connect-query-config.type';
import { type RelationConnectQueryFieldsByEntityIndex } from 'src/engine/cms-orm/entity-manager/types/relation-nested-query-fields-by-entity-index.type';
import {
  CMSORMException,
  CMSORMExceptionCode,
} from 'src/engine/cms-orm/exceptions/cms-orm.exception';
import { formatCompositeField } from 'src/engine/cms-orm/utils/format-data.util';
...
const computeRecordToConnectCondition = (
  ...
  if (
    ...
    throw new CMSORMException(
      ...
      CMSORMExceptionCode.CONNECT_NOT_ALLOWED,
      {
...
const checkUniqueConstraintFullyPopulated = (
  ...
  if (!hasUniqueConstraintFieldFullyPopulated) {
    throw new CMSORMException(
      ...
      CMSORMExceptionCode.CONNECT_UNIQUE_CONSTRAINT_ERROR,
      {
