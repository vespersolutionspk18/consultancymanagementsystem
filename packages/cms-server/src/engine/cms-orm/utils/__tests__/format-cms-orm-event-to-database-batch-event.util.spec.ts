...
import { type ObjectRecordUpdateEvent } from 'cms-shared/database-events';
...
describe('formatCMSOrmEventToDatabaseBatchEvent', () => {
  ...
  describe('UPDATED action', () => {
    it('should throw CMSORMException when no matching before entity is found in array of beforeEntities', () => {
      const afterEntities = [
        {
          id: 'record-1',
      ...
      try {
        ...
        expect(error).toBeInstanceOf(CMSORMException);
        expect((error as CMSORMException).code).toBe(
          CMSORMExceptionCode.ORM_EVENT_DATA_CORRUPTED,
        );
        expect((error as CMSORMException).message).toBe(
    ...
    it('should successfully create update events when matching before entities are found and in the right order', () => {
      ...
      const result = formatCMSOrmEventToDatabaseBatchEvent({
        action: DatabaseEventAction.UPDATED,
        objectMetadataItem: flatObjectMetadata,
        flatFieldMetadataMaps,
        workspaceId: mockWorkspaceId,
