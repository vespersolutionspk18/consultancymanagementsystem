...
import { RELATION_NESTED_QUERY_KEYWORDS } from 'cms-shared/constants';
import {
  type EntityTarget,
  type ObjectLiteral,
...
} from 'src/engine/cms-orm/entity-manager/types/relation-nested-query-fields-by-entity-index.type';
import {
  CMSORMException,
  CMSORMExceptionCode,
} from 'src/engine/cms-orm/exceptions/cms-orm.exception';
import { formatConnectRecordNotFoundErrorMessage } from 'src/engine/cms-orm/relation-nested-queries/utils/formatConnectRecordNotFoundErrorMessage.util';
import { type WorkspaceSelectQueryBuilder } from 'src/engine/cms-orm/repository/workspace-select-query-builder';
import { computeRelationConnectQueryConfigs } from 'src/engine/cms-orm/utils/compute-relation-connect-query-configs.util';
import { createSqlWhereTupleInClause } from 'src/engine/cms-orm/utils/create-sql-where-tuple-in-clause.utils';
import { extractNestedRelationFieldsByEntityIndex } from 'src/engine/cms-orm/utils/extract-nested-relation-fields-by-entity-index.util';
import { getAssociatedRelationFieldName } from 'src/engine/cms-orm/utils/get-associated-relation-field-name.util';
import { getObjectMetadataFromEntityTarget } from 'src/engine/cms-orm/utils/get-object-metadata-from-entity-target.util';
import { getRecordToConnectFields } from 'src/engine/cms-orm/utils/get-record-to-connect-fields.util';
        ...
        if (
          ...
          if (recordToConnect.length !== 1) {
            ...
            throw new CMSORMException(
              errorMessage,
              CMSORMExceptionCode.CONNECT_RECORD_NOT_FOUND,
              {
                userFriendlyMessage,
              },
            );
