...
export class DnsManagerService {
  ...
  constructor(
    ...
    if (this.cmsConfigService.get('CLOUDFLARE_API_KEY')) {
      this.cloudflareClient = new Cloudflare({
        apiToken: this.cmsConfigService.get('CLOUDFLARE_API_KEY'),
      });
    }
  }
      ...
      records: [
        ...
        },
        {
          ...
          value:
            ...
            `${hostname}.${this.cmsConfigService.get('CLOUDFLARE_DCV_DELEGATION_ID')}.dcv.cloudflare.com`,
        },
      ],
    };
  }
  ...
  private getZoneId(options?: DnsManagerOptions): string {
    return options?.isPublicDomain
      ? this.cmsConfigService.get('CLOUDFLARE_PUBLIC_DOMAIN_ZONE_ID')
      : this.cmsConfigService.get('CLOUDFLARE_ZONE_ID');
  }
