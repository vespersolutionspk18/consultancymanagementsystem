...
import { getAbsoluteUrl } from 'cms-shared/utils';
...
export class CallWebhookJob {
  ...
  private async callWebhook(data: CallWebhookJobData): Promise<void> {
    ...
    try {
      ...
      if (secret) {
        headers['X-CMS-Webhook-Timestamp'] = Date.now().toString();
        headers['X-CMS-Webhook-Signature'] = this.generateSignature(
          payloadWithoutSecret,
          secret,
          headers['X-CMS-Webhook-Timestamp'],
        );
        headers['X-CMS-Webhook-Nonce'] = crypto
          .randomBytes(16)
          .toString('hex');
      }
