...
describe('EmailDriverFactory', () => {
  ...
  describe('buildConfigKey', () => {
    it('should return "logger" for logger driver', () => {
      ...
      expect(cmsConfigService.get).toHaveBeenCalledWith('EMAIL_DRIVER');
    });

    it('should return smtp config key for smtp driver', () => {
      jest.spyOn(cmsConfigService, 'get').mockReturnValue(EmailDriver.SMTP);
      jest
    ...
    it('should throw error for unsupported driver', () => {
      jest.spyOn(cmsConfigService, 'get').mockReturnValue('invalid-driver');
