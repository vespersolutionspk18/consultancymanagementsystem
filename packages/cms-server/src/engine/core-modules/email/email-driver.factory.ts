...
export class EmailDriverFactory extends DriverFactoryBase<EmailDriverInterface> {
  ...
  protected buildConfigKey(): string {
    const driver = this.cmsConfigService.get('EMAIL_DRIVER');
  ...
  protected createDriver(): EmailDriverInterface {
    ...
    switch (driver) {
      ...
      case EmailDriver.SMTP: {
        const host = this.cmsConfigService.get('EMAIL_SMTP_HOST');
        const port = this.cmsConfigService.get('EMAIL_SMTP_PORT');
        const user = this.cmsConfigService.get('EMAIL_SMTP_USER');
        const pass = this.cmsConfigService.get('EMAIL_SMTP_PASSWORD');
        const noTLS = this.cmsConfigService.get('EMAIL_SMTP_NO_TLS');
