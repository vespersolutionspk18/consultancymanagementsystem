...
@ArgsType()
class GetTimelineThreadsFromProjectIdArgs {
  @Field(() => UUIDScalarType)
  projectId: string;
...
export class TimelineMessagingResolver {
  ...
  @Query(() => TimelineThreadsWithTotalDTO)
  async getTimelineThreadsFromProjectId(
    @AuthUser() user: UserEntity,
    @AuthWorkspace() workspace: WorkspaceEntity,
    @Args()
    { projectId, page, pageSize }: GetTimelineThreadsFromProjectIdArgs,
  ) {
    const workspaceMember = await this.userService.loadWorkspaceMember(
      user,
      workspace,
    ...
    const timelineThreads =
      await this.getMessagesFromPersonIdsService.getMessagesFromProjectId(
        workspaceMember.id,
        projectId,
        workspace.id,
        page,
