...
import { CMS_STANDARD_APPLICATION } from 'src/engine/workspace-manager/cms-standard-application/constants/cms-standard-applications';
...
export class ApplicationService {
  ...
  async findWorkspaceCMSStandardAndCustomApplicationOrThrow({
    ...
    const cmsStandardApplicationId =
      flatApplicationMaps.idByUniversalIdentifier[
        CMS_STANDARD_APPLICATION.universalIdentifier
      ];

    if (!isDefined(cmsStandardApplicationId)) {
      throw new ApplicationException(
        `Could not find workspace cms standard applicationId in cache ${workspaceId}`,
        ApplicationExceptionCode.APPLICATION_NOT_FOUND,
      );
    }

    const cmsStandardFlatApplication =
      flatApplicationMaps.byId[cmsStandardApplicationId];
    const workspaceCustomFlatApplication =
      flatApplicationMaps.byId[workspace.workspaceCustomApplicationId];

    if (
      !isDefined(cmsStandardFlatApplication) ||
  ...
  async createCMSStandardApplication(
    {
      workspaceId,
    ...
    const cmsStandardApplication = await this.create(
      {
        ...CMS_STANDARD_APPLICATION,
