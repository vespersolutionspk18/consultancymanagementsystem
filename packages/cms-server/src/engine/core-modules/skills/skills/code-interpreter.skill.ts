...
## Calling CMS Tools from Python (MCP Bridge)

A \`cms\` helper is automatically available in your code. Use it to call any CMS tool directly from Python:
...
# Create a record
result = cms.call_tool('create_company_record', {
    'data': {'name': 'Acme Corp', 'domainName': {'primaryLinkUrl': 'acme.com'}}
})
print(f"Created company: {result['id']}")

# Update a record
cms.call_tool('update_person_record', {
    'id': 'person-uuid',
    'data': {'jobTitle': 'CEO'}
})

# List available tools
tools = cms.list_tools()
for tool in tools:
