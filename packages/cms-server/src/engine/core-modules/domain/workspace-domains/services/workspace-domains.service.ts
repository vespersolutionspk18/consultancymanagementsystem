...
export class WorkspaceDomainsService {
  ...
  private async getDefaultWorkspace() {
    if (this.cmsConfigService.get('IS_MULTIWORKSPACE_ENABLED')) {
      throw new Error(
  ...
  private getCMSWorkspaceUrl(subdomain: string) {
    const url = this.domainServerConfigService.getFrontUrl();

    url.hostname = this.cmsConfigService.get('IS_MULTIWORKSPACE_ENABLED')
  ...
  getSubdomainAndCustomDomainFromWorkspaceFallbackOnDefaultSubdomain(
    ...
    if (!workspace) {
      return {
        subdomain: this.cmsConfigService.get('DEFAULT_SUBDOMAIN'),
        customDomain: null,
    ...
    return {
      customUrl:
        isCustomDomainEnabled && customDomain
          ...
          : undefined,
      subdomainUrl: this.getCMSWorkspaceUrl(subdomain),
    };
  }
