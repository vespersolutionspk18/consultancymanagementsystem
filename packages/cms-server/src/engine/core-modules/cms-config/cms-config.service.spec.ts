...
import { type ConfigVariables } from 'src/engine/core-modules/cms-config/config-variables';
...
jest.mock(
  'src/engine/core-modules/cms-config/constants/config-variables-masking-config',
  () => ({
...
jest.mock(
  'src/engine/core-modules/cms-config/utils/is-env-only-config-var.util',
  () => ({
    isEnvOnlyConfigVar: jest.fn(),
  }),
);

type CMSConfigServicePrivateProps = {
...
const setupTestModule = async (isDatabaseConfigEnabled = true) => {
  ...
  return {
    service: module.get<CMSConfigService>(CMSConfigService),
...
const setPrivateProps = (
  ...
  props: Partial<CMSConfigServicePrivateProps>,
) => {
