...
import ms from 'ms';
import { PasswordUpdateNotifyEmail } from 'cms-emails';
...
export class AuthService {
  ...
  async checkIsEmailVerified(isEmailVerified: boolean) {
    const isEmailVerificationRequired = this.cmsConfigService.get(
    ...
    const apps = [
      {
        ...
        redirectUrl:
          ...
          NodeEnvironment.DEVELOPMENT
            ...
            : `https://${this.cmsConfigService.get(
                'CHROME_EXTENSION_ID',
              )}.chromiumapp.org/`,
      },
    ];
  ...
  async updatePassword(
    ...
    await this.emailService.send({
      from: `${this.cmsConfigService.get(
        'EMAIL_FROM_NAME',
      )} <${this.cmsConfigService.get('EMAIL_FROM_ADDRESS')}>`,
      to: user.email,
      subject,
      text,
      html,
