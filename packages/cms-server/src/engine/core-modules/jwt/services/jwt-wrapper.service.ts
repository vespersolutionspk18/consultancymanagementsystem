...
export class JwtWrapperService {
  ...
  verifyJwtToken(
    ...
    try {
      ...
      // See this PR for context -> https://github.com/cmshq/cms/pull/16504
  ...
  generateAppSecret(type: JwtTokenTypeEnum, appSecretBody: string): string {
    const appSecret = this.cmsConfigService.get('APP_SECRET');
  ...
  generateAppSecretLegacy(): string {
    const accessTokenSecret = this.cmsConfigService.get(
      'ACCESS_TOKEN_SECRET',
    );
