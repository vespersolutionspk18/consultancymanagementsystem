...
import { SendApprovedAccessDomainValidation } from 'cms-emails';
...
export class ApprovedAccessDomainService {
  ...
  async sendApprovedAccessDomainValidationEmail(
    ...
    const emailTemplate = SendApprovedAccessDomainValidation({
      ...
      serverUrl: this.cmsConfigService.get('SERVER_URL'),
    ...
    await this.emailService.send({
      from: `${sender.name.firstName} ${sender.name.lastName} (via CMS) <${this.cmsConfigService.get('EMAIL_FROM_ADDRESS')}>`,
      to,
    ...
    return crypto
      ...
      .update(
        JSON.stringify({
          ...
          domain: approvedAccessDomain.domain,
          key: this.cmsConfigService.get('APP_SECRET'),
        }),
      )
      .digest('hex');
  }
