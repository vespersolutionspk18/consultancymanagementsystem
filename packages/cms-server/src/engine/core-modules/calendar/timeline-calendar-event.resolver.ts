...
@ArgsType()
class GetTimelineCalendarEventsFromProjectIdArgs {
  @Field(() => UUIDScalarType)
  projectId: string;
...
export class TimelineCalendarEventResolver {
  ...
  @Query(() => TimelineCalendarEventsWithTotalDTO)
  async getTimelineCalendarEventsFromProjectId(
    @Args()
    {
      projectId,
      page,
      pageSize,
    }: GetTimelineCalendarEventsFromProjectIdArgs,
    @AuthWorkspaceMemberId() workspaceMemberId: string,
    @AuthWorkspace() workspace: WorkspaceEntity,
  ) {
    const timelineCalendarEvents =
      await this.timelineCalendarEventService.getCalendarEventsFromProjectId(
        {
          currentWorkspaceMemberId: workspaceMemberId,
          projectId,
          workspaceId: workspace.id,
