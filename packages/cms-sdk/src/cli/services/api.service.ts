...
export class ApiService {
  ...
  constructor() {
    ...
    this.client.interceptors.request.use(async (config) => {
      const cmsConfig = await this.configService.getConfig();

      config.baseURL = cmsConfig.apiUrl;

      if (cmsConfig.apiKey) {
        config.headers.Authorization = `Bearer ${cmsConfig.apiKey}`;
      }
    ...
    this.client.interceptors.response.use(
      ...
      (error) => {
        if (error.response?.status === 401) {
          console.error(
            chalk.red(
              'Authentication failed. Please run `cms auth login` first.',
            ),
          );
          ...
          console.error(
            chalk.red('Cannot connect to CMS server. Is it running?'),
          );
        }
        throw error;
      },
