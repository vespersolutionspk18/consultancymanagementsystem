...
export class GenerateService {
  ...
  async generateClient(appPath: string): Promise<void> {
    ...
    console.log(chalk.blue('📦 Generating CMS client...'));
    ...
    await this.injectCMSClient(output);

    console.log(chalk.green('✓ Client generated successfully!'));
    console.log(chalk.gray(`Generated files at: ${outputPath}`));
  }

  private async injectCMSClient(output: string) {
    const cmsClientContent = `

// ----------------------------------------------------
// ✨ Custom CMS client (auto-injected)
// ----------------------------------------------------
...
`;
    ...
    await fs.appendFile(join(output, 'index.ts'), cmsClientContent);
  }
}
