...
type PersistedConfig = CMSConfig & {
  profiles?: Record<string, CMSConfig>;
};
...
export class ConfigService {
  ...
  constructor() {
    this.configPath = path.join(os.homedir(), '.cms', 'config.json');
  }
  ...
  async setConfig(config: Partial<CMSConfig>): Promise<void> {
    const raw = await this.readRawConfig();
    const profile = this.getActiveWorkspaceName();
  ...
  private getDefaultConfig(): CMSConfig {
    return {
